...
### 1.6. Nhóm: Chi nhánh / Đơn vị (`current_branch`)
Dùng để chèn thông tin của chi nhánh/đơn vị mà người dùng soạn thảo đang thuộc về.

> **Điều kiện:** Người dùng phải đã được Admin gán vào một Chi nhánh trong trang hồ sơ cá nhân.

**Thông tin cơ bản:**
- **Tên chi nhánh**: `{{ current_branch.name }}`

**Trường mở rộng (BRANCH dynamic fields):**
Tương tự USER_EXT, Admin có thể cấu hình thêm các trường tùy chỉnh cho đối tượng BRANCH (địa chỉ, MST, số điện thoại, ngày thành lập...).
Tên biến là "Mã (Placeholder)" được định nghĩa trong Admin > Trường thông tin.
- **Địa chỉ**: `{{ current_branch.dia_chi }}`
- **Mã số thuế**: `{{ current_branch.ma_so_thue }}`
- **Số điện thoại**: `{{ current_branch.so_dien_thoai }}`
- **Ngày thành lập**: `{{ current_branch.ngay_thanh_lap }}`

> **Lưu ý:** Để sử dụng biến `current_branch.*`, Admin cần:
> 1. Vào **Admin > Loại Đối tượng**, tạo loại mới với mã `BRANCH`.
> 2. Vào **Admin > Trường Thông tin** → thêm các trường dữ liệu thuộc đối tượng BRANCH.
> 3. Vào **Dữ liệu gốc** (với quyền root), tạo danh sách chi nhánh.
> 4. Enduser vào trang **Hồ sơ cá nhân** để chọn chi nhánh mình thuộc về.
